...
- Bodies with `material: 'basic'` use `SolidBodyMaterial` only when `bodyTextures.ts` has no approved texture for that body. The deployed reduced major moons Ganymede, Callisto, Rhea, Titan, Iapetus, Umbriel, Titania, Oberon, and Triton now use NASA-sourced texture assets through the same shared world-space lighting path as the standard textured planets.
...
- Milestone 11 is complete. The deployed reduced major-moon profile passed local and GitHub Pages assessment with one-year JSON chunks; chunk-size and file-format optimization are deferred to Milestone 13 only if restored fast-moon measurements justify it.
