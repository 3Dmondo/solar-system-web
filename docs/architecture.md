...
- Bodies with `material: 'basic'` use `SolidBodyMaterial` only when `bodyTextures.ts` has no approved texture for that body. Ganymede, Callisto, Rhea, Titan, Iapetus, Umbriel, Titania, Oberon, and Triton use NASA-sourced texture assets through the same shared world-space lighting path as the standard textured planets. Restored fast moons without dedicated local texture assets use the solid-color fallback.
