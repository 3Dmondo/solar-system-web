...
- When the external source is enabled, the runtime loads generated manifest and chunk assets from the configured data base URL and uses the committed `public/ephemeris/body-metadata.json` snapshot by default, with an optional explicit metadata-URL override. The snapshot now covers the deployed Sun, planets, Moon, and full expanded major-moon set, including satellite radii, shape fields, pole orientation, rotation, and GM-derived facts where the NAIF text kernels provide them.
...
- The expanded major-moons preview convention is `public/ephemeris/generated-expanded-major-moons/`, staged from sibling `SpiceNet` output by `scripts/Stage-ExpandedMajorMoonsPreview.ps1` and kept out of git except for `.gitkeep`. The staging helper requires `-AllowMilestone13FastMoons` for restored fast-moon profiles. `SpiceNet` supports fractional-day `SampleDays` values without a manifest schema change, and `scripts/Package-ReducedMajorMoonsReleaseAsset.ps1` packages staged profiles into the GitHub release asset consumed by Pages.
...
- `SolarSystemExperience` owns the focused target state, coarse-pointer detection, the simulation clock, and the resolved body-catalog hook. Focus targets include the global overview, individual bodies, and selectable planetary-system targets such as `system:jupiter`.
...
- `ExperienceHud` is now informational: it shows the current target label, overview or focused-view state, runtime loading or error messages, and a read-only focused-body facts drawer when a body is selected. It defaults open outside mobile portrait layouts, defaults closed on mobile portrait, updates that default when orientation changes, and can be toggled from the `i` rail button or closed from the panel's `x` button. On wide-enough layouts it stays visible while Help, Jump, Frame, or Layers popovers are open; in mobile portrait it temporarily hides behind those popovers and restores after they close if it was open before. The HUD is its own scrollable pointer target so short screens can use wheel, scrollbar, or touch pan to read longer facts. Selector and playback controls live in dedicated floating surfaces.
...
- The `Jump to` selector is grouped by parent system, removes the old Quick picks section, keeps `Overview` as the first row for focused-mode recovery, and shows selectable system rows for loaded parent-satellite systems.
...
- The reference-frame selector derives its names-only menu from loaded satellite systems. The deployed expanded catalog exposes SSB plus loaded parent-centered frames such as Earth, Mars, Jupiter, Saturn, Uranus, and Neptune when their moons are present.
...
- The resolved body metadata also carries compact generated physical facts for the focused-body drawer: mean radius, approximate surface gravity, approximate bulk density, and NAIF provenance when the generated metadata provides them. Bodies without generated physical fields still show Wikipedia-sourced description copy and a clear fallback provenance.
...
- `BodyId`, `ViewTargetId`, and `BodyDefinition` live in `src/features/solar-system/domain/body.ts`. `ViewTargetId` supports `overview`, individual bodies, and `system:*` planetary-system targets derived from loaded parent-satellite pairs. `BodyId` is derived from `BODY_REGISTRY`, and `BodyMetadata` carries optional `poleDirectionRender` (north-pole unit vector in render space) and `angularVelocityRadPerSec` (signed physical spin rate) alongside radius and focus offset when generated metadata is complete enough to derive them.
