...
- close the first Milestone 5.1 runtime optimization task for the current scope while preserving its history for possible reopening if later runtime work changes the cost profile
- refine focused camera target lock and post-focus stabilization at high playback rates so the orbit target stays on the live body center during and after focus changes
