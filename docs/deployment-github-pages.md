...
- Release asset packaging uses `scripts/Package-ReducedMajorMoonsReleaseAsset.ps1`, which validates the reduced body set, zips `manifest.json` plus `chunk-*.json`, and writes a SHA-256 sidecar for upload to GitHub Releases.
